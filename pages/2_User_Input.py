...
st.set_page_config(page_title="Black Scholes Pricing Model (User Input)", layout="wide")
...
with st.sidebar:
    ...
    time_to_expiry_days = col2.number_input("Time to Expiry (in days)", min_value=0.00, value=252)
    volatility = col2.number_input("Volatility", min_value=0, value=20.00)
time_to_expiry = time_to_expiry_days/252
...
with st.sidebar:
    ...
    min_vol = st.slider("Min Volatility", 0.00, 100.00, max((volatility*0.5),0.00))
    max_vol = st.slider("Max Volatility",0.00, 100.00, min((volatility*1.5),100.00))
